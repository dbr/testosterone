...
def detail(name_dotted):
    """Given a dotted TestCase name, return a detail report on its tests.
    """

    # Get a TestSuite for a single TestCase.
    # ======================================

    if '.' not in name_dotted:
        raise NameError("Please provide a full dotted name of a TestCase.")
    try:
        module_name, testcase_name = name_dotted.rsplit('.', 1)
        module = load(module_name)
        testcase = getattr(module, testcase_name)
    except:
        raise ImportError("Unable to find %s." % name_dotted)
    if not isinstance(testcase, (type, types.ClassType)):
        raise TypeError("%s is not a TestCase." % name_dotted)
...
class _Summarize:
    ...
    def print_body(self):
        ...
        for name, testcase in self.__testcases:
            ...
            # =======================
            # If we aren't running the tests, include all testcases in the
            # report. If we are, then only report on testcases with
            # failures/errors, unless told not to be quiet.
