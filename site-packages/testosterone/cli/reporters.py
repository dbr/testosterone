...
class _Summarize:
    ...
    def __call__(base, quiet=True, recursive=True, run=True, stopwords=()):

        modules = self.get_modules(base, recursive)
        self.runner = unittest.TextTestRunner(dev_null()) # swallow unittest output

        self.report = StringIO()

        self.print_header()
        totals = self.print_body()
        self.print_footer(*totals)

        return self.report.getvalue()


    def _get_modules(base, recursive):
