...
class _Summarize:
    ...
    the output; if False, they are. All submodules will also be included in the
    output, unless their name contains a stopword. If run is False, then no
    statistics on passes, failures, and errors will be available, and the output
    for each will be a dash character ('-'). run defaults to True.
    ...
    def __call__(self, base, run=True, stopwords=()):
        """
        """
        self.base = base
        self.run = run
