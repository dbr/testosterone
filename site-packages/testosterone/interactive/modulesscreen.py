...
class ModulesScreen:
    ...
    def draw_list(self):
        ...
        for i in range(self.toprows+1, self.toprows+1+self.listing.rows):
            self.win.addstr(i,1,' '*(c1w+2))
            self.win.addstr(i,c1w+5,' '*(c2w+2))

        #if not self.summary.names: onion?
        #    self.win.refresh()
        #    return
        ...
        parent = ''
        for index, row  in self.listing:
            parent = self.draw_row(index, row, parent)

        selected_index = self.listing.selected + self.listing.offset
        self.selected = self.summary.names[selected_index]
    ...
    def draw_row(self, index, row):
        ...
        #ints = (self.start, self.end, j)
        #logger.debug(' '.join([str(i) for i in ints]))

        pass5, fail, err, all, show, fresh = self.summary.get(index)
        ...
        self.win.addstr(j,3,shortname,color)
        ...
        self.win.addch(j,1,l,a)
        self.win.addch(j,self.W-1,r,a)
        ...
        w = self.W-c2w-1
        self.win.addstr(row,w,pass5.rjust(4),color)
        self.win.addstr(row,w+5,err.rjust(4),color)
        self.win.addstr(row,w+10,fail.rjust(4),color)
        self.win.addstr(row,w+15,all.rjust(4),color)
