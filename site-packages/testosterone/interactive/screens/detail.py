...
class DetailScreen(BaseScreen):
    ...
    def react(self, c):
        ...
        if c in (ord('q'), curses.KEY_BACKSPACE, ascii.ESC, ascii.BS):
            return self.summary
        elif c == ord('h'):
            return # return HelpScreen(self.iface)
        elif c in (curses.KEY_F5, ord(' ')):
            self.spinner(self.detail.refresh)
            ...
            self.draw_content()
            return
        elif c in (curses.KEY_ENTER, ascii.LF):
            if self.selected == '':
    ...
    def populate(self):
        ...
        """
        self.tests = ScrollArea(self.c1[0]+1, len(self.detail), self.toprows)
        self.populate_result()
    ...
    def draw_content(self):
        ...
        for i in range(self.toprows, self.toprows+self.tests.numrows):
            ...
            if self.tests.bar is None:
                self.win.addch(i,0,curses.ACS_VLINE)
            ...
            if self.result.bar is None:
                self.win.addch(i,W,curses.ACS_VLINE)
