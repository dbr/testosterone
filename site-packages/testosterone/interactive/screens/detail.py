...
class DetailScreen(BaseScreen):
    ...
    def react(self, c):
        ...
        if c in ( ord('q')                      # back to summary
                ...
                , ascii.BS
                , curses.KEY_LEFT):
            logger.debug(self.summary.selected)
            return self.summary
        elif c in (ord(' '), curses.KEY_F5):    # refresh
            self.spinner(self.refresh)
            if self.selected not in self.detail.names:
                self.selected = self.detail.names[0]
            self.populate_result()
            self.draw_content()
            return                              # open traceback
        elif c in (curses.KEY_ENTER, ascii.LF, curses.KEY_RIGHT):
            if self.selected == '':
    ...
    def populate(self):
        ...
        if self.tests is not None:
            if (   (self.tests.numrows == numrows)
               and (self.tests.numitems == numitems)
               and (self.tests.toprow == toprows)
                  ):
                pass
        else:
            self.tests = ScrollArea(numrows, numitems, toprows)
    ...
    def draw_content(self):
        ...
        for i in range(self.toprows, self.toprows+self.tests.numrows):
        ...
        if self.tests.numitems != 0:
            for index, rownum  in self.tests:
                testcase = self.draw_row(index, rownum)
            selected = ''
            if self.focus == TESTS:
