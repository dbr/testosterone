...
class SummaryScreen(BaseScreen):
    ...
    def react(self, c):
        ...
        # ====================
            ...
            self.run()
        elif c == ascii.LF:         # see detail
            return DetailScreen(self)
    ...
    def reload(self):
        self.summary = Summary(self.stopwords)
        traceback = self.spinner(self.refresh, self.base)
        if traceback:
            err = StandardError()
    ...
    def run(self):
        traceback = self.spinner(self.refresh, self.selected, run=True)
        if traceback:
            err = StandardError()
            err.traceback = traceback
            raise err
    ...
    def refresh(self, base, run=False):
        """Wrap summary.refresh() for error handling.
        """
        self.summary.refresh(base, run)
